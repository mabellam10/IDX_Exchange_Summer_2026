...
listing_flagged["flag_cols"] = listing_flagged[flag_cols].any(axis=1)

listing_filtered = listing_flagged[listing_flagged["flag_cols"] == False].copy()

#Part 5: Data Quality check and Stat comparison summary
print("\nDataset size and metric comparison (before vs. after)")
print(f"Original Row Count: {len(listing_flagged):,}")
print(f"Filtered Row Count: {len(listing_filtered):,}")
print(f"Total Rows Removed : {len(listing_flagged) - len(listing_flagged):,}")

comparison_rows = []

for col in target_cols:
    if col in listing_filtered.columns:
        before_median = listing_flagged[col].median()
        after_median = listing_filtered[col].median()
        before_mean = listing_flagged[col].mean()
        after_mean = listing_filtered[col].mean()
...
pd.DataFrame(comparison_rows).to_csv("listing_comparison.csv", index=False)
